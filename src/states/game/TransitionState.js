...
export default class TransitionState extends State {
	...
	constructor(type, onTransitionComplete = () => {}, colour = { r: 0, g: 0, b: 0 }) {
		...
		timer.tween(this, { alpha: this.endAlpha }, 1, Easing.linear, () => {
			stateStack.pop();
	 ...
	 */
	static fade(betweenEvent = () => stateStack.pop(), afterComplete = () => {}) {
		stateStack.push(
			new TransitionState(TransitionState.TYPE.In, () => {
				betweenEvent();
				stateStack.push(new TransitionState(TransitionState.TYPE.Out, afterComplete)); // added the afterComplete callback for more choices for ordering actions
			})
		);
	}
}
